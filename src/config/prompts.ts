...
固定输出格式：
{"mainEvent":{"eventType":"...","eventSubject":"...","eventAction":"...","eventObject":"...","eventDate":"YYYY-MM-DD|null"}|null,"events":[{"eventType":"...","eventSubject":"...","eventAction":"...","eventObject":"...","eventDate":"YYYY-MM-DD|null","oneLiner":"...","qualityScore":0}]}

输出要求：
1. events：拆出的子事件数组，最多 20 条；如果原文包含超过 20 条事件，只保留事实密度和新闻价值最高的前 20 条。逐个事件独立成新闻，每个事件必须能独立署名给具体主体、动作、对象。
...
7. oneLiner：1 句话中文事实摘要，最多 60 字，覆盖"主体 + 动作 + 对象 + 关键结果"。
8. qualityScore：0-100 整数，评估此子事件在原文中的事实密度和重要性。
9. mainEvent：若全文有清晰的"主事件"（如"X 公司发布 Y 模型"占主导），返回该事件的完整签名；如聚合里各事件地位均等没有主导事件，返回 null。
10. 所有文本字段默认中文；品牌名、产品名、专有名词可保留原文。
11. 最终只能输出合法 JSON 对象。`;
